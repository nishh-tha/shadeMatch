...
public class ShadeService {
    ...
    public List<MatchResult> findMatches(String hex) {
        ...
        for (Shade shade : allShades) {
            ...
            if (shade.getHex() == null || shade.getHex().isBlank()) {
                continue;
            }

            if (shade.getHex().equalsIgnoreCase(hex)) {
                continue;
            }
            ...
            try {
                ...
                double dist = deltaE(
                        target,
                        rgbToLab(hexToRgb(shade.getHex()))
                );

                // Convert Delta E distance to similarity %
                double similarity = Math.max(0, 100 - (dist * 2));

                String brand = shade.getBrand();

                // Keep only best match per brand
                if (!bestDistance.containsKey(brand)
                        || dist < bestDistance.get(brand)) {

                    bestDistance.put(brand, dist);
                    bestSimilarity.put(brand, similarity);
        ...
        return bestPerBrand.values()
                ...
                .sorted(Comparator.comparingDouble(
                        s -> bestDistance.get(s.getBrand())
                ))
                .map(shade -> new MatchResult(
                        shade,
                        Math.round(bestSimilarity.get(shade.getBrand()))
                ))
                .collect(Collectors.toList());
    }
